...
AI agent（或聊天机器人）把代码发给 colabmcp 时，命令文本往往要穿过各种会破坏引号的
传输层：IM 网关吞掉 `"`、智能引号转换把 `"` 变成 “ ”、shell 反复解析反斜杠。
代码到达内核时引号早已面目全非（"引号地狱"）。

`colabmcp exec` 用 **Samai Command Envelope（信封协议）** 解决：代码被包在纯 ASCII
...
### v2.2.0（最新）
- 🆕 新增 `colabmcp exec` 一发式命令，支持 Samai Command Envelope —— 为 AI agent 提供防引号地狱的代码投递（stdin 信封 / `--c64` base64url 参数 / 普通 `-c`）
...
### v2.1.0（最新）
- 🆕 新增 `/execute_stream` SSE 端点，用于实时输出
- 🆕 新增 `colabmcp stream` 命令，用于流式执行
- 🆕 新增 `colabmcp watch` 命令，用于服务器监控
- ✨ 改进流式任务的中断处理
- 🐛 修复 shell 命令输出流式问题

### v2.0.0
- 🆕 新增 `/interrupt` 端点 - 停止代码而不杀死服务器
- 🆕 新增 `/status` 端点 - 跟踪当前目录和执行状态
- 🆕 新增 `/history` 端点 - 查看命令执行历史
- ✨ 智能重复检测 `cd` 命令

### v1.0.0
- 初始版本
- 基础 notebook 执行（本地和远程）
- IPython magic 命令支持
- 逐 cell 流式输出
...
- 原始想法：[colabmcp](https://github.com/ctz168/colabmcp)
